...
function FrontBody({ volume }: { volume: Record<string, number> }) {
  ...
  const fill = (muscle: string) =>
    `rgba(37,119,255,${muscleOpacity(volume[muscle] ?? 0, max)})`;
  const filter = (muscle: string) => muscleGlow(volume[muscle] ?? 0, max);
...
export function MuscularBalanceCard() {
  ...
  if (loading) {
    return (
      <div className="h-[340px] rounded-3xl bg-card border border-white/[0.05] animate-pulse" />
    );
  }
  ...
  return (
    <div className="p-5 rounded-3xl bg-card border border-white/[0.06] shadow-card">
      <div className="flex items-center justify-between mb-5">
        <p className="text-[10px] font-bold uppercase tracking-[0.12em] text-white/25">
          Balance muscular · 7 días
        </p>
      ...
      {!hasData ? (
        <div className="flex items-center justify-center h-[240px] text-white/20 text-sm font-medium">
          Registra entrenos para ver tu balance
        </div>
      ) : (
        <div className="flex gap-4 items-center">
          {/* SVG body */}
          <div className="w-[80px] shrink-0 h-[200px]">
            {side === "front" ? (
      ...
      {hasData && (
        <div className="mt-4 flex flex-wrap gap-1.5">
          {RADAR_MUSCLES.filter((m) => (volume[m] ?? 0) > 0).map((muscle) => (
            <span
              key={muscle}
              className="text-[10px] font-semibold px-2.5 py-1 rounded-full bg-primary/[0.10] text-primary/80 border border-primary/[0.15]"
            >
